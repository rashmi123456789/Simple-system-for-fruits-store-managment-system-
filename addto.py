...
class addto_page():
    ...
    def addToStore(self,p,h,j,l,o,y):
        ...
        query="insert into recieve(date,company_name,user_id,quantity_kg,total_price,item_id) values('"+o.get()+"','"+h.get()+"','"+user_id+"','"+j.get()+"','"+l.get()+"','"+item_id+"'); "
        if quantity!=0:
            query4="update stores set quantity='"+str(quantity+int(j.get()))+"' , total_price='"+str(total_price+int(l.get()))+"' where item_id='"+item_id+"';"
        else:
            query4="insert into stores (item_id,item_name,quantity,total_price) values('"+item_id+"','"+p.get()+"','"+str(j.get())+"','"+str(l.get())+"');"
        print(query)
        print(query4)
        cur.execute(query)
        cur.execute(query4)
        ...
        i = Label(self.gui ,text="Item Added to store!!!")
    ...
    def GUI(self):
        ...
        #Button
        c = Button(self.gui, text="Add",command=lambda:self.addToStore(dropVarChanges,h,j,l,o,dropVarChanges1))
